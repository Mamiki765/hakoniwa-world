...
{
    private const SOURCE_KEY = 'roadmap-pr2-v1';

    private const TARGET_KEY = 'roadmap-pr6-v1';
    ...
    public function up(): void
    {
        ...
        if ($sourceId === null) {
            ...
            if (! is_array($sourceSettings)) {
                throw new RuntimeException('The immutable roadmap-pr2-v1 ruleset snapshot is missing.');
            }
            $sourceId = app(RulesetPublisher::class)->publish($sourceSettings)->id;
        }
    ...
    private function moveWorldAndQueueItems(int $fromRulesetId, int $toRulesetId): void
    {
        ...
        $toDefinitions = DB::table('command_definitions')->where('ruleset_version_id', $toRulesetId)
            ->pluck('id', 'key');
        if ($fromDefinitions->count() !== $toDefinitions->count()) {
