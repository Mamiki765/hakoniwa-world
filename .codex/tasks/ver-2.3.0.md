...
## C3 verified implementation result (2026-08-20)
...
- Added exact custom asset mappings only. No GIF binary was created or committed, and missing files resolve to the existing null-URL fallback.
- Added ruleset-owned standard split and hostless-full-killer-money policies. v1-v10 preserve the legacy split and event shape; Aoi's neutral-host fixture requests all 1,200億円 for the killer, applies capacity, creates no host meat, and increments the kill statistic once.
- The advanced manual has ten display-order-sorted rows. Its order and values are contract-tested directly from the shared fixture.
- Public detail and ranking each use one aggregate query plus one eager definition query for ten and twenty species; the ranking bound is unchanged across two Nations. Frontend ranking/detail tests render eleven species with accessible missing-asset fallback and bounded scrolling.
- Local verification: historical `migrate:fresh` through C3 passed; open-question contract remains 77 IDs (Decided 55, Deferred 17, Open 5); Pint passed all 442 PHP files; full-app PHPStan passed; frontend 129 tests/lint/typecheck/build passed; the 16-shard plan covered 108 test files with duplicate/missing/unexpected counts all zero and every shard passed (one existing skip), followed by a focused 38-test/483-assertion final self-audit run.
- C4 still owns actual Aoi/Zero spawn, movement, dispatch, and self-destruction. C5 still owns the formal v11 config/publication migration, World/data rebind, and release conversion.
...
C4 adds the nullable immutable request-ruleset provenance support column without updating any existing row; all newly authored single and bulk queue rows store the locked request ruleset. The fingerprint column was introduced immediately before v10 publication: pre-C4 rows remain null, and request provenance may remain v10 after a later safe execution-definition rebind. Before rebind, C5 attributes from source World, definition ruleset, release chronology, request key, target snapshot, and the existing hash-format constraint—not by recomputing the hash with the mutable queue position—and backfills safely attributable v10 rows across queued and terminal statuses. Any contradictory non-null row aborts publication; null historical fingerprints remain conflict-only. Duplicate lookup occurs before current-v11 selector validation. Only a matching stored request key with a C4-inspector-proved v10 request, stable `monster_dispatch`, and stored quantity 1 may normalize an omitted selector to 1 for fingerprint comparison. The original selector-less v10 payload can therefore return `duplicate = true` when its stored fingerprint matches; selector 2 conflicts, while a missing selector for a new or unproved request still fails normally.
...
## C4 implementation result and C5 handoff

C4 implements the approved behavior only through the inactive shared v11-shaped fixture. The runtime now has a closed ruleset-owned monster behavior resolver; the post-subsidence World Aoi stage with exact streams `global_disasters:aoi_inora:trigger:v1`, `global_disasters:aoi_inora:candidate:v1`, and `global_disasters:aoi_inora:hp:v1` plus durable spawn source `world_aoi_disaster`; water-only neutralizing Aoi movement; real hostless Aoi reward integration; immutable initial-island plan/apply with exact-dirty-cell rewardless displacement and stable reservation cell-ID lock order; one two-option `monster_dispatch` resolver used by backend, UI, fingerprint, execution cost, spawn, and events; request-ruleset provenance support plus a fail-closed v10 inspector; and Zero's pre-movement fixed-center rewardless nuclear action with no collateral chain. Old Bow uses the existing metadata safety policy and has no Zero key branch.

The only checkpoint-boundary shift is the expressly permitted nullable provenance support column and reusable inspector in C4. C4 leaves all historical values null and performs no backfill, definition rebind, World activation, or formal v11 publication. C5 must use that seam for an all-or-nothing safely attributable backfill/rebind and must preserve original fingerprint bytes. The dedicated collateral removal reason is `nuclear_self_destruct_blast`; the initiating Zero retains exact reason `nuclear_self_destruct`.

Checkpoint recovery verified that explicit v10 request provenance survives a queued execution-definition rebind to v11: the original selector-less quantity-1 retry converges and preserves the stored fingerprint byte-for-byte, selector 2 conflicts, and ambiguous provenance remains conflict-only. Authoring now rejects dispatch options whose monster definition is absent or non-dispatchable, requires the C4 Zero/Aoi baseline while still accepting 10+ species, and performs no client/database lookup. Spawn-turn-deferred monsters remain in the complete occupancy index but consume no action or movement draw, preventing another monster from entering the occupied cell and avoiding the database uniqueness rollback; existing v10 dispatch action/metric behavior remains unchanged.

The post-C3 review observation that authoring accepted `hp_variation = 19` and `natural_spawn_tier = 4` beyond the persisted database bounds is fixed in C4 with red regressions. The later explicit Owner wording contract retains `中立海域(X,Y)` for neutral Zero self-destruction. B-12 remains Open and untouched. Durable implementation details and verification boundaries are recorded in `product/docs/ver-2.3.0-c4-new-monsters.md`.
