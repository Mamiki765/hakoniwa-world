...
return new class extends Migration
{
    ...
    {
        // One unreleased 4.4.0 transition from the pre-4.4 schema. Keep the final
        // activation identity; do not replay intermediate development schemas.
        $this->isolateSecretarySurfaceState();
        $this->createReceiptRollups();
        $this->preserveDurableReceiptDependencies();
        $this->extendGuideRequests();
        $this->extendAdministration();
        $this->extendUndergroundEquipment();
        $this->extendOtherworldProgress();
        $this->createSurfaceContent();

        // Publish/rebind only after every schema and data prerequisite exists.
        // Laravel keeps this activation in the same migration transaction.
    ...
    {
        // D1/D2/D4 are one release: create the final, initially empty table.
        // There is no already-verified intermediate prefix to reaggregate.
...
SQL);
    ...
    private function preserveDurableReceiptDependencies(): void
    {
        // Source IDs remain positive, exclusive provenance after receipt purge.
...
SQL);
        // Only the recorded first choice is evidence; never infer from a respec.
...
SQL);
    ...
    private function extendGuideRequests(): void
    {
        Schema::table('underground_intro_requests', function (Blueprint $table): void {
            // A battle purge must not delete an unverified request in another stream.
        ...
        // Set the final release operation contract once, not once per feature.
        DB::statement(<<<'SQL'
...
SQL);
    ...
    private function extendAdministration(): void
    {
        ...
        // Preserve amounts/history; start the existing-grant deadline at upgrade.
        ...
        Schema::table('worlds', function (Blueprint $table): void {
            // Delayed completion times are not evidence of the actual calendar.
...
SQL);
        ...
        // Legacy rewards retain their saved NULL index and occupy effective index 1.
...
SQL);
    ...
    private function extendOtherworldProgress(): void
    {
    ...
    private function createSurfaceContent(): void
    {
    ...
    public function down(): void
    {
        throw new RuntimeException(
            'The 4.4.0 schema, durable assets and Ruleset activation are forward-only; restore the verified pre-migration backup.',
        );
    }
};
