...
**4.2.0は海洋アップデートとテスト基盤の改善で区切る。PR #9は `release/4.2.0 → main` の最終release PRであり、まだマージしない。** 統合HEADの横断レビューで報告されたP1は局所修正し、P2はOwnerが共有通常視界を正しい仕様として確認した。詳細は[4.2.0最終レビュー](../releases/4.2.0-final-review.md)。
...
- 撃沈した海賊人口から、ミサイルなら50%、戦艦なら100%を難民受入候補にする。人口上限等による未受入分と実受入数を区別する。受入serviceは変更cell IDを返し、戦艦は共有turn-local cellを即時更新、ミサイルは既存changed-cell経路へ載せる。
...
- 遠方キラッはプレイヤー向け表示で、探索船AIへの遠隔座標配信ではない。探索船AIは領土・別の自国船を含む自国の通常視界を共有情報として使い、その中から各探索船に最も近い宝を追跡する。宝ごとに担当船を排他的に割り当てるfleet assignmentは行わない。
...
詳しい操作説明は[港と船](../manual/ships.md)、[地上の秘書](../manual/secretary.md)。探索船自身の視界半径は3だが、自動追跡に使う通常視界はNation内で共有する。20%の遠隔キラッはこの共有情報へ含めない。

## 4. 統合レビュー指摘への対応

| ID | 優先度 | 対応 |
|---|---|---|
| F1 | P1 | 修正済み。難民受入cell IDをcombat結果へ返し、戦艦は共有MapCellをrefresh、ミサイルは既存changed-cell refreshへ伝える |
| F2 | P2 | Owner確認により指摘を取り下げる。Nationの通常視界は無線等で共有し、20%遠隔キラッだけをAI候補から除外する現行実装を維持する |

詳細・元のsource行番号は[最終レビュー](../releases/4.2.0-final-review.md)。F1は別の難民決算engineや全map再読込を追加せず、既存のchanged-cell同期へ接続した。F2はcode変更を行わず、Owner確定後のmanualとhandoffだけを現行挙動へ合わせた。
...
F1修正branchを差分reviewし、releaseへ反映後に#9のHEADを再解決する。実際に変えた境界の代表focusedで止め、Full・Surface全件・Underground全件を自動再実行しない。
