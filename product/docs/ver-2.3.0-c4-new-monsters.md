...
The corrected pre-deploy Aoi movement contract reuses the ordinary monster candidate, protection, defense-contact, occupancy, and movement pass. Aoi alone may also enter sea, shallow, seabed-base, and seabed-oil-field cells. Mountain, mine, monument, Capital, bounds, occupied cells, and every other target blocked by the ordinary movement contract remain blocked; defense contact retains the ordinary rewardless removal and one huge-meteor-equivalent blast. A successful destination is normalized atomically to sea, owner null, population zero, and no destructible facility. A changed destination emits one movement plus one public/affected-Nation trample event with pre-impact owner, terrain, facility, and coordinates; unchanged empty sea emits movement only. Because the Aoi occupies the newly created sea, it can continue invading adjacent legal land on later turns. `MonsterTurnBatch`, model versions, and changed chunks are updated through the same movement pass without an extra draw.
