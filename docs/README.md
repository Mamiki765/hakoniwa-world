...
| Underground RPG alpha | [`open-questions.md`](open-questions.md)のE-01/UG-01〜04、[`3.0.0-alpha-underground.md`](roadmap/3.0.0-alpha-underground.md)、[`underground-combat-laboratory.md`](architecture/underground-combat-laboratory.md)、`product/config/underground/balance/foundation-v0.json` | `App\Domain\Underground`、`App\Application\Underground`、`UndergroundProfile`、`tests/Underground`。UG-02はSecretary-owned persistence foundationを決定済み | [`post-release-backlog.md`](future-systems/post-release-backlog.md)はidea provenanceだけ。laboratory観測win rateをplayer balance/Tutorialへ流用しない。UG-03/04前にplayer API/UI、persistent run、facility/surface bridgeを実装しない |
