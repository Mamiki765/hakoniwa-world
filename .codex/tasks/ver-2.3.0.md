...
C4 implements the approved behavior only through the inactive shared v11-shaped fixture. The runtime now has a closed ruleset-owned monster behavior resolver; the post-subsidence World Aoi stage with dedicated trigger/candidate/HP streams; water-only neutralizing Aoi movement; real hostless Aoi reward integration; immutable initial-island plan/apply with exact-dirty-cell rewardless displacement; one two-option `monster_dispatch` resolver used by backend, UI, fingerprint, execution cost, spawn, and events; request-ruleset provenance support plus a fail-closed v10 inspector; and Zero's pre-movement fixed-center rewardless nuclear action with no collateral chain. Old Bow uses the existing metadata safety policy and has no Zero key branch.
...
The post-C3 review observation that authoring accepted `hp_variation = 19` and `natural_spawn_tier = 4` beyond the persisted database bounds is fixed in C4 with red regressions. B-12 remains Open and untouched. Durable implementation details and verification boundaries are recorded in `product/docs/ver-2.3.0-c4-new-monsters.md`.
