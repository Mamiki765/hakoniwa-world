...
| Underground RPG alpha | [`open-questions.md`](open-questions.md)のE-01/UG-01〜04、[`3.0.0-alpha-underground.md`](roadmap/3.0.0-alpha-underground.md)、[`underground-combat-laboratory.md`](architecture/underground-combat-laboratory.md)、`product/config/underground/balance/foundation-v0.json` | `App\Domain\Underground`、`UndergroundBalanceSimulator`、`tests/Underground`。surface接続時だけcurrent Secretary/User authority | [`post-release-backlog.md`](future-systems/post-release-backlog.md)はidea provenanceだけ。laboratory観測win rateをplayer balance/Tutorialへ流用しない。UG gate前にschema/API/UI/surface bridgeを実装しない |
