...
return new class extends Migration
{
    ...
    private const SOURCE_KEY = 'hakoniwa-2s-plus-v4';

    private const TARGET_KEY = 'hakoniwa-2s-plus-v5';
    ...
    public function up(): void
    {
        ...
        if (! is_array($sourceSettings) || ! is_array($targetSettings)) {
            throw new RuntimeException('The immutable v4 or v5 production ruleset snapshot is missing.');
        }
    ...
    public function down(): void
    {
        throw new RuntimeException(
            'The hakoniwa-2s-plus-v5 production migration is forward-only; restore through an explicit reviewed conversion.',
        );
    }
    ...
    private function assertApprovedTargetDiff(array $source, array $target): void
    {
        ...
        if ($target !== $expected) {
            throw new RuntimeException('hakoniwa-2s-plus-v5 contains changes outside the approved sea-edge removal.');
        }
    }
...
SQL, [$toRulesetId, $world->id, $fromRulesetId]);
    ...
    private function assertNoUnresolvedNextTurnRun(object $world): void
    {
        ...
        if ($run !== null) {
            throw new RuntimeException(
                "Refusing v5 migration with unresolved non-dry TurnRun {$run->id}, "
    ...
    private function assertDefinitionSetsMatch(string $table, int $fromRulesetId, int $toRulesetId): void
    {
        ...
        if ($source !== $target || count($source) !== count(array_unique($source))
            ...
            throw new RuntimeException("v4 and v5 have different or ambiguous {$table} stable-key sets.");
        }
    }
