...
function mapCell(overrides: Partial<MapCell> = {}): MapCell {
    return {
        q: 0, r: 0, terrain: 'plain', terrain_name: '平地', facility: null, facility_name: null,
        ...
        aria_label: 'q 0 r 0 平地 所有 地図国', version: 1, updated_at: null,
        ...overrides,
    };
}
...
describe('staggered square-image map', () => {
    it('renders completed and optional overlay images, fallback, selection and six-way keyboard input', async () => {
        ...
        const fallback = mapCell({
            q: 1, r: 0, terrain: 'forest', terrain_name: '森', display_name: '森', owner_nation_id: null, owner_name: null,
            asset: { key: 'tile.forest', url: null, available: false, fallback_label: '森', fallback_style: 'tile-forest' },
            overlays: [], aria_label: 'q 1 r 0 森 所有 中立',
        });
        const wrapper = mount(HexMap, { props: {
            cells: [selected, fallback], selected, capital: { q: 0, r: 0 }, nationId: 1, mapSpaceId: 1,
        ...
        expect(wrapper.text()).toContain('q=0, r=0');
    ...
    it('does not leak secret facility data when given the public forest representation', async () => {
        ...
        const publicForest = mapCell({
            ...
            overlays: [], aria_label: 'q 0 r 0 森 所有 他国', owner_name: '他国',
        });
        const wrapper = mount(HexMap, { props: {
            cells: [publicForest], selected: publicForest, capital: { q: 0, r: 0 }, nationId: 2, mapSpaceId: 1,
            loading: false, error: null, emptyChunks: [],
        } });
        await flushPromises();
        ...
        expect(wrapper.find('.map-cell').attributes('aria-label')).toBe('q 0 r 0 森 所有 他国');
