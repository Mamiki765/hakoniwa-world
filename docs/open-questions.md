...
- Status: Decided
- Implemented: Partially; application `3.3.0`までに正式intro・契約・4 growth path・通常探索・growth/STP・有限SP・player Skill Tree、Trial 1・覚醒、Nation-owned施設とread-only surface bridge、正式equipment・装備Shop・アクセサリー3枠・500枠宝物庫、浅層と黒晶洞、Item Lv・rarity・affix・generated drop、案内人の部屋と複合再振りを実装済み。custom AI、party、marketは未実装。
...
- Status: Decided
- Implemented: Yes; application `3.3.0`。current authenticated User自身のSecretary画面からだけ入れるserver-authoritative finite-state intro、通常探索、growth settlement、status/STP、有限SP、戦技・護身・祝福Skill Tree、Trial 1・覚醒、正式equipment、装備Shop、アクセサリー3枠、500枠宝物庫、浅層と黒晶洞のgenerated drop、案内人の部屋と複合再振りを実装する。Tutorial rewardはXP +5 / 欠片0で一度だけ、scripted lossはprogression/economyへ影響しない。開発環境の「力試し（α）」も報酬なしとし、productionではmain payloadとplaytest APIの双方を閉じる。通常探索、Trial、取得済みactive/passive、実際の装備instanceはcanonical pure core/modelとPR103 battle historyを再利用する。
