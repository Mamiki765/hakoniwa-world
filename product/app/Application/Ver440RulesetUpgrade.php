...
final readonly class Ver440RulesetUpgrade
{
    public const SOURCE_KEY = 'hakoniwa-2s-plus-v26';

    public const SOURCE_VERSION = 26;

    public const SOURCE_CHECKSUM = '791ec7754ba794660fff3e27c1b287a481096055cf125d1080a6fe31c6f3d10b';

    public const TARGET_KEY = 'hakoniwa-2s-plus-v27';

    public const TARGET_VERSION = 27;

    public const TARGET_CHECKSUM = 'f353eb5a409657809a1e3ebdcc5fa6dcd7f700ba6b0a6bc5165313cd32c1d3f4';
    ...
    {
        $sourceSettings = require config_path('hakoniwa/rulesets/hakoniwa-2s-plus-v26.php');
        $targetSettings = require config_path('hakoniwa/rulesets/hakoniwa-2s-plus-v27.php');
        if (($sourceSettings['key'] ?? null) !== self::SOURCE_KEY
            ...
            throw new RuntimeException('The exact immutable v26 and authored v27 Rulesets required by the ver 4.4.0 upgrade are missing or changed.');
        }
        ...
        if (! $world instanceof World) {
            return DB::transaction(function () use ($targetSettings): string {
                ...
                if (World::query()->exists()) {
                    throw new RuntimeException('A World appeared while publishing fresh-install v27.');
                }
                $this->catalogInstaller->install($targetSettings);
                $this->publisher->publish($targetSettings);

                return 'fresh_install_current_v27';
            }, 1);
        }
    ...
    private function upgradeLockedWorld(World $advisoryWorld, array $sourceSettings, array $targetSettings): string
    {
        ...
        if ($worlds->count() !== 1 || (int) $worlds->first()->id !== (int) $advisoryWorld->id
            ...
            throw new RuntimeException('The v27 upgrade supports exactly one locked shared-world.');
        }
        /** @var World $world */
        $world = $worlds->first();
        ...
        if ($existingTarget instanceof RulesetVersion
            ...
            return 'already_current_v27';
        }

        $source = $this->publisher->assertPublished($sourceSettings);
        if ((int) $world->ruleset_version_id !== (int) $source->id) {
            throw new RuntimeException('The v27 upgrade requires the mutable shared-world to be exact v26.');
        }
        ...
        if (DB::table('worlds')->where('id', $world->id)
            ...
            throw new RuntimeException('shared-world changed during the exact v26 to v27 upgrade.');
        }
        ...
        if ($this->captureTrigger('monster_instances', self::MONSTER_TRIGGER) !== $monsterTrigger
            ...
            throw new RuntimeException('A gameplay integrity trigger changed during the v27 upgrade.');
        }
        ...
        foreach ($protected as $name => $digest) {
            if ($digest !== $afterProtected[$name]) {
                throw new RuntimeException("The v27 upgrade changed protected data: {$name}.");
            }
        }
        $this->recordActivation($world);

        return 'production_v26_to_v27';
    }
    ...
    private function assertDefinitionKeys(int $sourceId, int $targetId): void
    {
        foreach (['command_definitions', 'production_definitions', 'monster_definitions'] as $table) {
            ...
            if ($source === [] || $source !== $target) {
                throw new RuntimeException("{$table} stable keys differ across exact v26 to v27.");
            }
        }
    }
...
SQL, [$worldId, $targetId, $worldId, $targetId, $worldId, $targetId]);
        if ((int) DB::table('worlds')->where('id', $worldId)->value('ruleset_version_id') !== $targetId
            ...
            throw new RuntimeException('Exact v27 activation postconditions failed.');
        }
    }
    ...
    private function recordActivation(World $world): void
    {
        ...
        DB::table('audit_events')->insert([
            ...
            'message' => null,
            'visibility' => 'admin',
            'event_type' => 'ruleset.v27_activated',
            'severity' => 'info',
...
SQL, [$table, $trigger]);
        if ($row === null || $row->tgenabled !== 'O') {
            throw new RuntimeException("{$trigger} must be enabled for the v27 upgrade.");
        }
