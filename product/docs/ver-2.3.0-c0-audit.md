...
| membership removal | Nation abandonment holds only `WorldMutationLock`; it deletes membership/queue, removes monsters, rewrites territory/Nation, and emits an event without an unresolved-next-TurnRun guard | `product/app/Application/NationAbandonmentService.php:45-224` |
...
by User ID. The only current membership-set writers are `NationCreationService` (insert) and
`NationAbandonmentService` (delete). Both must acquire the user lock before their target
`WorldMutationLock` and hold both through commit. While holding the target World lock and before any
state write, each locks/reloads the World and rejects a next non-dry pending/running/failed/blocked
TurnRun at `current_turn + 1`. For registration this precedes membership, Secretary, starter Item, and
island writes; for abandonment it precedes membership/queue deletion, monster removal, cell/Nation
rewrite, and event insertion. The World lock serializes the check against TurnRunner creation/retry.
Any future membership add/reactivate/remove/deactivate path must join this lock and guard. Equipment
holds the user lock before enumerating Worlds through its own commit. The global order is user-membership
lock, then World locks by ID, then DB rows. TurnRunner takes only its World lock and never waits for the
user lock, so this adds no reverse edge.
...
| membership-set mutation | registration/abandonment each take one User advisory, target World lock/row, and one indexed next-TurnRun guard before existing work | create/delete races with equipment, each unresolved status, no-run/success paths, no partial registration or abandonment state |
...
- Nation abandonment and equipment serialize in both orders; abandonment rejects all four unresolved
  statuses before mutation and preserves membership, queue, monsters, cells, active Nation/resources,
  capital, policies, chunks, and events exactly;
