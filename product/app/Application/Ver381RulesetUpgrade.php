...
final readonly class Ver381RulesetUpgrade
{
    public const SOURCE_KEY = 'hakoniwa-2s-plus-v21';

    public const SOURCE_VERSION = 21;

    public const SOURCE_CHECKSUM = '2d7187deae540cb06dd066770272d42734a95d93fe9e18d41d99d40ae9e4e964';

    public const TARGET_KEY = 'hakoniwa-2s-plus-v22';

    public const TARGET_VERSION = 22;

    public const TARGET_CHECKSUM = '4f3289fe5d9066d1d3025b976b9531dd0780868e8b4166f1f4eb2cf3fc7998bb';
    ...
    {
        $sourceSettings = require config_path('hakoniwa/rulesets/hakoniwa-2s-plus-v21.php');
        $targetSettings = require config_path('hakoniwa/rulesets/hakoniwa-2s-plus-v22.php');
        if (($sourceSettings['key'] ?? null) !== self::SOURCE_KEY
            ...
            throw new RuntimeException('The exact immutable v21 and authored v22 Rulesets required by the ver 3.8.1 upgrade are missing or changed.');
        }
        ...
        if (! $world instanceof World) {
            return DB::transaction(function () use ($targetSettings): string {
                ...
                if (World::query()->exists()) {
                    throw new RuntimeException('A World appeared while publishing fresh-install v22.');
                }
                $this->catalogInstaller->install($targetSettings);
                $this->publisher->publish($targetSettings);
                $this->rebaseShipOperations($targetSettings);

                return 'fresh_install_current_v22';
            }, 1);
        }
    ...
    private function upgradeLockedWorld(World $advisoryWorld, array $sourceSettings, array $targetSettings): string
    {
        ...
        if ($worlds->count() !== 1 || (int) $worlds->first()->id !== (int) $advisoryWorld->id
            ...
            throw new RuntimeException('The v22 upgrade supports exactly one locked shared-world.');
        }
        /** @var World $world */
        $world = $worlds->first();
        ...
        if ($existingTarget instanceof RulesetVersion
            ...
            return 'already_current_v22';
        }

        $source = $this->publisher->assertPublished($sourceSettings);
        if ((int) $world->ruleset_version_id !== (int) $source->id) {
            throw new RuntimeException('The v22 upgrade requires the mutable shared-world to be exact v21.');
        }
        ...
        if (DB::table('worlds')->where('id', $world->id)
            ...
            throw new RuntimeException('shared-world changed during the exact v21 to v22 upgrade.');
        }
        ...
        if ($this->captureTrigger('monster_instances', self::MONSTER_TRIGGER) !== $monsterTrigger
            ...
            throw new RuntimeException('A gameplay integrity trigger changed during the v22 upgrade.');
        }

        $skillRebase = $this->rebaseShipOperations($targetSettings);
        $this->assertPostconditions((int) $world->id, (int) $target->id);
        $afterProtected = $this->protectedDigests();
        foreach ($protected as $name => $digest) {
            if ($digest !== $afterProtected[$name]) {
                throw new RuntimeException("The v22 upgrade changed protected data: {$name}.");
            }
        }
        $this->recordActivation($world, $skillRebase);

        return 'production_v21_to_v22';
    }
...
SQL, [$targetId, $worldId, $sourceId]);
    ...
    private function assertDefinitionKeys(int $sourceId, int $targetId): void
    {
        foreach (['command_definitions', 'production_definitions', 'monster_definitions'] as $table) {
            ...
            if ($source === [] || $source !== $target) {
                throw new RuntimeException("{$table} stable keys differ across exact v21 to v22.");
            }
        }
    }
    ...
    /**
     * @param  array<string, mixed>  $targetSettings
     * @return array{rows_changed: int, levels_gained: int}
     */
    private function rebaseShipOperations(array $targetSettings): array
    {
        $definition = $targetSettings['secretary']['skills'][SecretarySkillCatalog::SHIP_OPERATIONS] ?? null;
        if (! is_array($definition)
            || ($definition['level_requirement'] ?? null) !== [
                'basis' => 'next_level_linear',
                'multiplier' => 100,
            ]) {
            throw new RuntimeException('The v22 Ship operations progression definition is invalid.');
        }

        $changed = 0;
        $levelsGained = 0;
        $rows = DB::table('secretary_skills')
            ->where('skill_key', SecretarySkillCatalog::SHIP_OPERATIONS)
            ->orderBy('id')->lockForUpdate()->get(['id', 'level', 'experience']);
        foreach ($rows as $row) {
            $result = $this->skillProgression->advance(
                $definition,
                (int) $row->level,
                (int) $row->experience,
                0,
            );
            if ($result['level'] === (int) $row->level
                && $result['experience'] === (int) $row->experience) {
                continue;
            }
            if (DB::table('secretary_skills')->where('id', $row->id)->update([
                'level' => $result['level'],
                'experience' => $result['experience'],
                'updated_at' => now(),
            ]) !== 1) {
                throw new RuntimeException('A Ship operations skill changed during the v22 progression rebase.');
            }
            $changed++;
            $levelsGained += $result['levels_gained'];
        }

        return ['rows_changed' => $changed, 'levels_gained' => $levelsGained];
    }
...
SQL, [$worldId, $targetId, $worldId, $targetId, $worldId, $targetId]);
        if ((int) DB::table('worlds')->where('id', $worldId)->value('ruleset_version_id') !== $targetId
            ...
            throw new RuntimeException('Exact v22 activation postconditions failed.');
        }
    }
    ...
    private function protectedDigests(): array
    {
        return [
            ...
            'other_secretary_skills' => $this->queryDigest(
                DB::table('secretary_skills')->where('skill_key', '<>', SecretarySkillCatalog::SHIP_OPERATIONS),
            ),
        ];
    }
    ...
    /** @param array{rows_changed: int, levels_gained: int} $skillRebase */
    private function recordActivation(World $world, array $skillRebase): void
    {
        $now = now();
        DB::table('audit_events')->insert([
            ...
            'metadata' => json_encode([
                ...
                'ship_operations_progression_rebased' => $skillRebase,
            ], JSON_THROW_ON_ERROR),
            'occurred_at' => $now,
            'created_at' => $now,
            'updated_at' => $now,
...
SQL, [$table, $trigger]);
        if ($row === null || $row->tgenabled !== 'O') {
            throw new RuntimeException("{$trigger} must be enabled for the v22 upgrade.");
        }
