...
| Underground RPG alpha | [`open-questions.md`](open-questions.md)のE-01/UG-01〜04、[`3.0.0-alpha-underground.md`](roadmap/3.0.0-alpha-underground.md)、[`underground-combat-laboratory.md`](architecture/underground-combat-laboratory.md)、`product/config/underground/balance/foundation-v0.json` | `App\Domain\Underground`、`App\Application\Underground`、`UndergroundProfile`/intro state、`tests/Underground`。UG-03はapplication 3.0.0-alpha.1のfirst-player Tutorial導線まで決定済み | [`post-release-backlog.md`](future-systems/post-release-backlog.md)はidea provenanceだけ。laboratory観測win rateをplayer balanceへ流用しない。UG-04前にparty、facility、surface bridgeを実装せず、未決定のnormal hunt/Trial/shop/equipmentを公開しない |
