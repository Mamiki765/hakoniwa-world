...
{
    private const SOURCE_KEY = 'hakoniwa-2s-plus-v9';

    private const TARGET_KEY = 'hakoniwa-2s-plus-v10';
    ...
    public function up(): void
    {
        ...
        if (! is_array($sourceSettings) || ! is_array($targetSettings)) {
            throw new RuntimeException('The immutable v9 or v10 production ruleset snapshot is missing.');
        }

        $stage = $targetSettings['turn_processing']['food']['production_overflow_resolution_stage'] ?? null;
        unset($targetSettings['turn_processing']['food']['production_overflow_resolution_stage']);
        $targetSettings['key'] = self::SOURCE_KEY;
        $targetSettings['version'] = 9;
        if ($targetSettings !== $sourceSettings
            || $stage !== 'after_population_nutrition_consumption') {
            throw new RuntimeException('hakoniwa-2s-plus-v10 contains changes outside the approved food overflow stage contract.');
        }

        DB::transaction(function () use ($sourceSettings): void {
            ...
            $this->moveLiveReferences($source->id, $target->id);
        });
    }
    ...
    public function down(): void
    {
        throw new RuntimeException(
            'The hakoniwa-2s-plus-v10 production migration is forward-only; restore through an explicit reviewed conversion.',
        );
    }
    ...
    private function moveLiveReferences(int $fromRulesetId, int $toRulesetId): void
    {
        $world = app(SecretaryV1MigrationSafetyGuard::class)
            ->lockAndAssertNoUnresolvedNextTurnRun('v10 migration');
        if ($world === null) {
            return;
        }
        if (! in_array((int) $world->ruleset_version_id, [$fromRulesetId, $toRulesetId], true)) {
            throw new RuntimeException('shared-world is attached to an unexpected ruleset; refusing an implicit v10 migration.');
        }
...
SQL, [$toRulesetId, $world->id, $fromRulesetId]);
    ...
    private function assertDefinitionSetsMatch(string $table, int $fromRulesetId, int $toRulesetId): void
    {
        ...
        if ($source !== $target || count($source) !== count(array_unique($source))) {
            throw new RuntimeException("v9 and v10 have different or ambiguous {$table} stable-key sets.");
        }
    }
};
