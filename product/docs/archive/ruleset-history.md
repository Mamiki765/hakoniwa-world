...
| roadmap Ruleset | PR単位でpublishされたMVP開発中の完全payload | migration/rebuild/historyに必要なimmutable authored snapshot |
...
| migration snapshot/guard | source/target checksum、definition set、fingerprint expectations | migration-only。normal turn runtimeへ移さない |
...
These files are not abandoned drafts. They are migration inputs and historical authored snapshots. No roadmap source is a 2.3.1 deletion candidate.
...
| 5 | `hakoniwa-2s-plus-v5.php` | `4d1a004332b79b298460c0316c6ec00972a27517e01079f2378fc5de78591ab6` | removes the sea-edge population gameplay dependency | `2026_08_14_000000_publish_hakoniwa_2s_plus_v5.php` | immutable history; legacy sea-edge branch remains only when the setting exists |
...
| 9 | `hakoniwa-2s-plus-v9.php` | `78b55d34ce3148eb1e4b6dd97939468cee9df508d28f4084947a09cdd10fd883` | `normal_monster_stage = after_ordinary_surface_cell_events`; same cell order, no extra shuffle | `2026_08_17_000000_publish_hakoniwa_2s_plus_v9.php` | immutable history; runtime keeps pre-v9 same-pass branch |
| 10 | `hakoniwa-2s-plus-v10.php` | `6a0f5354f8894081bacdb8eaaba328d3e4ee80a2c4136819b16cfa924f485dc1` | post-nutrition food overflow stage contract | `2026_08_19_010000_publish_hakoniwa_2s_plus_v10.php` | immutable history and exact v11 migration source |
| 11 | `hakoniwa-2s-plus-v11.php` | `5c65c49ed3fd623375f004815ec6bba0b2f67524f61f0638c6fe528fe9599db8` | Secretary Old Bow/Ring effects, explicit monster behavior/reward/display, Aoi/Zero, two-option dispatch selector | `2026_08_21_010000_publish_hakoniwa_2s_plus_v11.php` and `RulesetV11MigrationService` | configured current gameplay and immutable history |

## Runtime and retry interpretation

- `config/hakoniwa.php` registers every authored roadmap/formal payload explicitly so migrations and publication equality checks are deterministic.
...
- version-aware runtime branches still preserve old semantics for migration chains, focused historical contexts, restore/rehearsal, and a same-ruleset retry before a version migration is allowed to proceed.
...
Production evidence needed to remove historical runtime support is listed in `product/docs/post-2.3.0-simplification-handoff.md`. It is absent from this offline cleanup, so no compatibility source or branch is removed.

## Test and migration-only sources

`tests/Support/V11SecretaryItemRulesetFixture.php` reads formal v11 and changes only the inactive test identity. It is not a never-published gameplay proposal. `RulesetV11MigrationService` embeds source/target checksum and exact live-reference/fingerprint expectations; those values are a migration guard, not another configurable Ruleset.

No other never-published Ruleset, obsolete duplicate, or unknown payload was found in the current repository and file history audit. That finding authorizes no deletion.
