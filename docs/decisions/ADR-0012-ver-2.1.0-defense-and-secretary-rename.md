...
The defense facility owner's Nation state does not disable this capability. A real defense owned by an `active`, `dormant_frozen`, `dormant_contestable`, or `sunken_archived` Nation covers an otherwise eligible nearby impact in the same way. This owner-scope decision is independent from the target-cell boundary: when the impact cell itself belongs to a protected dormant or archived Nation state, dormant target protection still resolves first and the impact is a complete no-op.
