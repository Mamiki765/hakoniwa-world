...
final class FirstProductionReleaseTest extends TestCase
{
    ...
    public function test_current_schema_and_v12_are_the_fresh_install_baseline(): void
    {
        ...
        $published = RulesetVersion::query()->where('key', 'hakoniwa-2s-plus-v12')->firstOrFail();
        $this->assertSame('hakoniwa-2s-plus-v12', config('hakoniwa.ruleset.key'));
        $this->assertSame(['hakoniwa-2s-plus-v12'], array_keys(config('hakoniwa.published_rulesets')));
        $this->assertSame(['hakoniwa-2s-plus-v12'], RulesetVersion::query()->pluck('key')->all());
