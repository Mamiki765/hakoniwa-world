...
final readonly class Ver390RulesetUpgrade
{
    public const SOURCE_KEY = 'hakoniwa-2s-plus-v22';

    public const SOURCE_VERSION = 22;

    public const SOURCE_CHECKSUM = '4f3289fe5d9066d1d3025b976b9531dd0780868e8b4166f1f4eb2cf3fc7998bb';

    public const TARGET_KEY = 'hakoniwa-2s-plus-v23';

    public const TARGET_VERSION = 23;

    public const TARGET_CHECKSUM = '9086ee835107c887d01dd81d79e92660de51af98993f1c8f3f70331eb056f141';
    ...
    /** @var list<string> */
    private const NEW_COMMAND_KEYS = [
        'build_central_bank',
    ...
    public function run(): string
    {
        ...
        $targetSettings = require config_path('hakoniwa/rulesets/hakoniwa-2s-plus-v23.php');
        if (($sourceSettings['key'] ?? null) !== self::SOURCE_KEY
            ...
            throw new RuntimeException('The exact immutable v22 and authored v23 Rulesets required by the ver 3.9.0 upgrade are missing or changed.');
        }
        ...
        if (! $world instanceof World) {
            return DB::transaction(function () use ($targetSettings): string {
                ...
                if (World::query()->exists()) {
                    throw new RuntimeException('A World appeared while publishing fresh-install v23.');
                }
                $this->catalogInstaller->install($targetSettings);
                $this->publisher->publish($targetSettings);

                return 'fresh_install_current_v23';
            }, 1);
        }
    ...
    private function upgradeLockedWorld(World $advisoryWorld, array $sourceSettings, array $targetSettings): string
    {
        ...
        if ($worlds->count() !== 1 || (int) $worlds->first()->id !== (int) $advisoryWorld->id
            ...
            throw new RuntimeException('The v23 upgrade supports exactly one locked shared-world.');
        }
        /** @var World $world */
        $world = $worlds->first();
        ...
        if ($existingTarget instanceof RulesetVersion
            ...
            return 'already_current_v23';
        }

        $source = $this->publisher->assertPublished($sourceSettings);
        if ((int) $world->ruleset_version_id !== (int) $source->id) {
            throw new RuntimeException('The v23 upgrade requires the mutable shared-world to be exact v22.');
        }
        ...
        if (DB::table('worlds')->where('id', $world->id)
            ...
            throw new RuntimeException('shared-world changed during the exact v22 to v23 upgrade.');
        }
        ...
        if ($this->captureTrigger('monster_instances', self::MONSTER_TRIGGER) !== $monsterTrigger
            ...
            throw new RuntimeException('A gameplay integrity trigger changed during the v23 upgrade.');
        }
        ...
        foreach ($protected as $name => $digest) {
            if ($digest !== $afterProtected[$name]) {
                throw new RuntimeException("The v23 upgrade changed protected data: {$name}.");
            }
        }
        $this->recordActivation($world);

        return 'production_v22_to_v23';
    }
    ...
    private function assertDefinitionKeys(int $sourceId, int $targetId): void
    {
        foreach (['production_definitions', 'monster_definitions'] as $table) {
            ...
            if ($source === [] || $source !== $target) {
                throw new RuntimeException("{$table} stable keys differ across exact v22 to v23.");
            }
        }

        $source = DB::table('command_definitions')->where('ruleset_version_id', $sourceId)->orderBy('key')->pluck('key')->all();
        $target = DB::table('command_definitions')->where('ruleset_version_id', $targetId)->orderBy('key')->pluck('key')->all();
        $expected = [...$source, ...self::NEW_COMMAND_KEYS];
        sort($expected, SORT_STRING);
        if ($source === [] || $target !== $expected) {
            throw new RuntimeException('command_definitions must add only the five approved v23 commands.');
        }
    }
...
SQL, [$worldId, $targetId, $worldId, $targetId, $worldId, $targetId]);
        if ((int) DB::table('worlds')->where('id', $worldId)->value('ruleset_version_id') !== $targetId
            ...
            throw new RuntimeException('Exact v23 activation postconditions failed.');
        }
    }
...
SQL, [$table, $trigger]);
        if ($row === null || $row->tgenabled !== 'O') {
            throw new RuntimeException("{$trigger} must be enabled for the v23 upgrade.");
        }
