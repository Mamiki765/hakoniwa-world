...
final class RulesetV3MigrationTest extends TestCase
{
    ...
    public function test_production_v2_territory_queue_migrates_through_v3_then_message_board(): void
    {
        ...
        $this->v4Migration()->up();
        $v4 = $world->fresh()->rulesetVersion()->firstOrFail();
        $this->assertSame('hakoniwa-2s-plus-v4', $v4->key);
        $this->assertSame($v4->id, $item->fresh()->definition()->value('ruleset_version_id'));
        $this->assertSame('territory_expand', $item->fresh()->definition()->value('key'));
        $this->assertSame($preserved, Arr::except($item->fresh()->getAttributes(), ['command_definition_id']));

        $this->v5Migration()->up();
        $v5 = $world->fresh()->rulesetVersion()->firstOrFail();
        $this->assertSame('hakoniwa-2s-plus-v5', $v5->key);
        ...
        $this->v7Migration()->up();
        $v7 = $world->fresh()->rulesetVersion()->firstOrFail();
        $this->assertSame('hakoniwa-2s-plus-v7', $v7->key);
        $this->assertSame($v7->id, $item->fresh()->definition()->value('ruleset_version_id'));
        $this->assertSame('territory_expand', $item->fresh()->definition()->value('key'));
        $this->assertSame($preserved, Arr::except($item->fresh()->getAttributes(), ['command_definition_id']));

        $this->v8Migration()->up();
        $v8 = $world->fresh()->rulesetVersion()->firstOrFail();
        $this->assertSame('hakoniwa-2s-plus-v8', $v8->key);
        $this->assertSame($v8->id, $item->fresh()->definition()->value('ruleset_version_id'));
        $this->assertSame('territory_expand', $item->fresh()->definition()->value('key'));
        $this->assertSame($preserved, Arr::except($item->fresh()->getAttributes(), ['command_definition_id']));

        $this->v9Migration()->up();
        $v9 = $world->fresh()->rulesetVersion()->firstOrFail();
        $this->assertSame('hakoniwa-2s-plus-v9', $v9->key);
        $this->assertSame($v9->id, $item->fresh()->definition()->value('ruleset_version_id'));
        $this->assertSame('territory_expand', $item->fresh()->definition()->value('key'));
        $this->assertSame($preserved, Arr::except($item->fresh()->getAttributes(), ['command_definition_id']));

        $this->v10Migration()->up();
        $v10 = $world->fresh()->rulesetVersion()->firstOrFail();
        $this->assertSame('hakoniwa-2s-plus-v10', $v10->key);
        $this->assertSame($v10->id, $item->fresh()->definition()->value('ruleset_version_id'));
        $this->assertSame('territory_expand', $item->fresh()->definition()->value('key'));
        $this->assertSame($preserved, Arr::except($item->fresh()->getAttributes(), ['command_definition_id']));
    ...
    private function v4Migration(): object
    {
