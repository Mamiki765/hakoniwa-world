...
final readonly class Ver393RulesetUpgrade
{
    public const SOURCE_KEY = 'hakoniwa-2s-plus-v24';

    public const SOURCE_VERSION = 24;

    public const SOURCE_CHECKSUM = 'a7c021756a4daff6ed8b769dffb69b830a345a322b04d919801ff2eb2aa27c92';

    public const TARGET_KEY = 'hakoniwa-2s-plus-v25';

    public const TARGET_VERSION = 25;

    public const TARGET_CHECKSUM = 'c03af0ca57f167207740ad5bc5e201568335b9c45d417c0c865440a9548967de';
    ...
    {
        $sourceSettings = require config_path('hakoniwa/rulesets/hakoniwa-2s-plus-v24.php');
        $targetSettings = require config_path('hakoniwa/rulesets/hakoniwa-2s-plus-v25.php');
        if (($sourceSettings['key'] ?? null) !== self::SOURCE_KEY
            ...
            throw new RuntimeException('The exact immutable v24 and authored v25 Rulesets required by the ver 3.9.3 upgrade are missing or changed.');
        }
        ...
        if (! $world instanceof World) {
            return DB::transaction(function () use ($targetSettings): string {
                ...
                if (World::query()->exists()) {
                    throw new RuntimeException('A World appeared while publishing fresh-install v25.');
                }
                $this->catalogInstaller->install($targetSettings);
                $this->publisher->publish($targetSettings);

                return 'fresh_install_current_v25';
            }, 1);
        }
    ...
    private function upgradeLockedWorld(World $advisoryWorld, array $sourceSettings, array $targetSettings): string
    {
        ...
        if ($worlds->count() !== 1 || (int) $worlds->first()->id !== (int) $advisoryWorld->id
            ...
            throw new RuntimeException('The v25 upgrade supports exactly one locked shared-world.');
        }
        /** @var World $world */
        $world = $worlds->first();
        ...
        if ($existingTarget instanceof RulesetVersion
            ...
            return 'already_current_v25';
        }

        $source = $this->publisher->assertPublished($sourceSettings);
        if ((int) $world->ruleset_version_id !== (int) $source->id) {
            throw new RuntimeException('The v25 upgrade requires the mutable shared-world to be exact v24.');
        }
        ...
        if (DB::table('worlds')->where('id', $world->id)
            ...
            throw new RuntimeException('shared-world changed during the exact v24 to v25 upgrade.');
        }
        ...
        if ($this->captureTrigger('monster_instances', self::MONSTER_TRIGGER) !== $monsterTrigger
            ...
            throw new RuntimeException('A gameplay integrity trigger changed during the v25 upgrade.');
        }
        ...
        foreach ($protected as $name => $digest) {
            if ($digest !== $afterProtected[$name]) {
                throw new RuntimeException("The v25 upgrade changed protected data: {$name}.");
            }
        }
        $this->recordActivation($world);

        return 'production_v24_to_v25';
    }
    ...
    private function assertDefinitionKeys(int $sourceId, int $targetId): void
    {
        foreach (['command_definitions', 'production_definitions', 'monster_definitions'] as $table) {
            ...
            if ($source === [] || $source !== $target) {
                throw new RuntimeException("{$table} stable keys differ across exact v24 to v25.");
            }
        }
    }
...
SQL, [$worldId, $targetId, $worldId, $targetId, $worldId, $targetId]);
        if ((int) DB::table('worlds')->where('id', $worldId)->value('ruleset_version_id') !== $targetId
            ...
            throw new RuntimeException('Exact v25 activation postconditions failed.');
        }
    }
...
SQL, [$table, $trigger]);
        if ($row === null || $row->tgenabled !== 'O') {
            throw new RuntimeException("{$trigger} must be enabled for the v25 upgrade.");
        }
