...
final readonly class Ver392RulesetUpgrade
{
    public const SOURCE_KEY = 'hakoniwa-2s-plus-v23';

    public const SOURCE_VERSION = 23;

    public const SOURCE_CHECKSUM = '9086ee835107c887d01dd81d79e92660de51af98993f1c8f3f70331eb056f141';

    public const TARGET_KEY = 'hakoniwa-2s-plus-v24';
    ...
    {
        $sourceSettings = require config_path('hakoniwa/rulesets/hakoniwa-2s-plus-v23.php');
        $targetSettings = require config_path('hakoniwa/rulesets/hakoniwa-2s-plus-v24.php');
        if (($sourceSettings['key'] ?? null) !== self::SOURCE_KEY
            ...
            throw new RuntimeException('The exact immutable v23 and authored v24 Rulesets required by the ver 3.9.2 upgrade are missing or changed.');
        }
        ...
        if (! $world instanceof World) {
            return DB::transaction(function () use ($targetSettings): string {
                ...
                if (World::query()->exists()) {
                    throw new RuntimeException('A World appeared while publishing fresh-install v24.');
                }
                $this->catalogInstaller->install($targetSettings);
                $this->publisher->publish($targetSettings);

                return 'fresh_install_current_v24';
            }, 1);
        }
    ...
    private function upgradeLockedWorld(World $advisoryWorld, array $sourceSettings, array $targetSettings): string
    {
        ...
        if ($worlds->count() !== 1 || (int) $worlds->first()->id !== (int) $advisoryWorld->id
            ...
            throw new RuntimeException('The v24 upgrade supports exactly one locked shared-world.');
        }
        /** @var World $world */
        $world = $worlds->first();
        ...
        if ($existingTarget instanceof RulesetVersion
            ...
            return 'already_current_v24';
        }

        $source = $this->publisher->assertPublished($sourceSettings);
        if ((int) $world->ruleset_version_id !== (int) $source->id) {
            throw new RuntimeException('The v24 upgrade requires the mutable shared-world to be exact v23.');
        }
        ...
        if (DB::table('worlds')->where('id', $world->id)
            ...
            throw new RuntimeException('shared-world changed during the exact v23 to v24 upgrade.');
        }
        ...
        if ($this->captureTrigger('monster_instances', self::MONSTER_TRIGGER) !== $monsterTrigger
            ...
            throw new RuntimeException('A gameplay integrity trigger changed during the v24 upgrade.');
        }
        ...
        foreach ($protected as $name => $digest) {
            if ($digest !== $afterProtected[$name]) {
                throw new RuntimeException("The v24 upgrade changed protected data: {$name}.");
            }
        }
        $this->recordActivation($world);

        return 'production_v23_to_v24';
    }
    ...
    private function assertDefinitionKeys(int $sourceId, int $targetId): void
    {
        foreach (['command_definitions', 'production_definitions', 'monster_definitions'] as $table) {
            ...
            if ($source === [] || $source !== $target) {
                throw new RuntimeException("{$table} stable keys differ across exact v23 to v24.");
            }
        }
    }
...
SQL, [$worldId, $targetId, $worldId, $targetId, $worldId, $targetId]);
        if ((int) DB::table('worlds')->where('id', $worldId)->value('ruleset_version_id') !== $targetId
            ...
            throw new RuntimeException('Exact v24 activation postconditions failed.');
        }
    }
...
SQL, [$table, $trigger]);
        if ($row === null || $row->tgenabled !== 'O') {
            throw new RuntimeException("{$trigger} must be enabled for the v24 upgrade.");
        }
