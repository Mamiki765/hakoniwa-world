...
return new class extends Migration
{
    ...
    private const SOURCE_KEY = 'hakoniwa-2s-plus-v3';

    private const TARGET_KEY = 'hakoniwa-2s-plus-v4';
    ...
    public function up(): void
    {
        ...
        if (! is_array($sourceSettings) || ! is_array($targetSettings)) {
            throw new RuntimeException('The immutable v3 or v4 production ruleset snapshot is missing.');
        }
    ...
    public function down(): void
    {
        throw new RuntimeException(
            'The hakoniwa-2s-plus-v4 production migration is forward-only; restore through an explicit reviewed conversion.',
        );
    }
    ...
    private function assertApprovedTargetDiff(array $source, array $target): void
    {
        ...
        if ($normalized !== $source) {
            throw new RuntimeException('hakoniwa-2s-plus-v4 contains changes outside the approved launch-base contracts.');
        }
    }
    ...
    private function moveLiveReferences(int $fromRulesetId, int $toRulesetId): void
    {
        ...
        if ($identity === null) {
            DB::statement('LOCK TABLE facility_definitions IN SHARE ROW EXCLUSIVE MODE');
            $this->prepareSeabedFacilityMetadata();
        ...
        DB::statement('LOCK TABLE facility_definitions IN SHARE ROW EXCLUSIVE MODE');
        DB::statement('LOCK TABLE map_cells IN SHARE ROW EXCLUSIVE MODE');
        ...
        if ((int) $world->ruleset_version_id === $toRulesetId) {
            ...
            $this->assertSeabedExperienceReady((int) $world->id);

            return;
        }

        $seabedFacilityId = $this->prepareSeabedFacilityMetadata();
        $this->assertNoNextTurnRun($world);
        $this->assertLiveReferencesUseRuleset((int) $world->id, $fromRulesetId, 'before migration');
        $this->prepareSeabedExperience((int) $world->id, $seabedFacilityId);
...
SQL, [$toRulesetId, $world->id, $fromRulesetId]);
    ...
    private function prepareSeabedFacilityMetadata(): int
    {
        $facility = DB::table('facility_definitions')->where('key', 'seabed_base')
            ->lockForUpdate()->first(['id', 'metadata']);
        if ($facility === null) {
            throw new RuntimeException('The seabed_base facility definition is missing.');
        }
        $metadata = json_decode((string) $facility->metadata, true, flags: JSON_THROW_ON_ERROR);
        $targetMetadata = [
        ...
        if ($metadata !== [] && ! $this->isTargetSeabedMetadata($metadata)) {
            throw new RuntimeException('seabed_base facility metadata is unexpected; refusing implicit repair.');
        }
        DB::table('facility_definitions')->where('id', $facility->id)->update([
            'metadata' => json_encode($targetMetadata, JSON_THROW_ON_ERROR),
            'updated_at' => now(),
        ]);
    ...
    private function prepareSeabedExperience(int $worldId, int $facilityId): void
    {
        $unexpected = DB::table('map_cells as cell')
            ->join('map_spaces as space', 'space.id', '=', 'cell.map_space_id')
            ->where('space.world_id', $worldId)
            ->where('cell.facility_definition_id', $facilityId)
            ->whereNotNull('cell.facility_experience')
            ->exists();
        if ($unexpected) {
            throw new RuntimeException('Existing shared-world seabed_base experience is unexpected before v4 migration.');
        }
        DB::update(<<<'SQL'
UPDATE map_cells cell
   SET facility_experience = 0,
       updated_at = CURRENT_TIMESTAMP
  FROM map_spaces space
 WHERE space.id = cell.map_space_id
   AND space.world_id = ?
   AND cell.facility_definition_id = ?
   AND cell.facility_experience IS NULL
SQL, [$worldId, $facilityId]);
    }

    private function assertSeabedExperienceReady(int $worldId): void
    {
        $facility = DB::table('facility_definitions')->where('key', 'seabed_base')->first(['id', 'metadata']);
        if ($facility === null) {
            throw new RuntimeException('The seabed_base facility definition is missing.');
        }
        $metadata = json_decode((string) $facility->metadata, true, flags: JSON_THROW_ON_ERROR);
        if (! $this->isTargetSeabedMetadata($metadata)) {
            throw new RuntimeException('seabed_base facility metadata does not match the v4 contract.');
        }
        $invalid = DB::table('map_cells as cell')
            ->join('map_spaces as space', 'space.id', '=', 'cell.map_space_id')
            ->where('space.world_id', $worldId)
            ->where('cell.facility_definition_id', $facility->id)
            ->where(fn ($query) => $query->whereNull('cell.facility_experience')
                ->orWhere('cell.facility_experience', '<', 0)
                ->orWhere('cell.facility_experience', '>', 200))
            ->exists();
        if ($invalid) {
            throw new RuntimeException('shared-world contains a seabed_base without valid v4 experience.');
        }
    }

    /** @param array<string, mixed> $metadata */
    private function isTargetSeabedMetadata(array $metadata): bool
    {
        return count($metadata) === 4
            && ($metadata['initial_experience'] ?? null) === 0
            && ($metadata['maximum_experience'] ?? null) === 200
            && ($metadata['level_thresholds'] ?? null) === [50, 200]
            && ($metadata['launch_capacity_by_level'] ?? null) === ['1' => 1, '2' => 2, '3' => 3];
    }
    ...
    private function assertNoNextTurnRun(object $world): void
    {
        ...
        if ($run !== null) {
            throw new RuntimeException(
                "Refusing v4 migration with non-dry TurnRun {$run->id}, target_turn={$run->target_turn}, status={$run->status}.",
            );
        }
    }
    ...
    private function assertDefinitionSetsMatch(string $table, int $fromRulesetId, int $toRulesetId): void
    {
        ...
        if ($source !== $target || count($source) !== count(array_unique($source))
            ...
            throw new RuntimeException("v3 and v4 have different or ambiguous {$table} stable-key sets.");
        }
    }
